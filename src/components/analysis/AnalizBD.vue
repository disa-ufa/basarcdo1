<template>
  <div>
    ...
    <div class="meta-info" v-if="!loading && !error && metaInfoBGU">
      ...
      >К добавлению: {{ newOsItems.length }}</span>
      <span
        class="item-count"
        v-if="!loading && !error && discrepancyMolItems.length > 0"
        style="margin-left: 15px; color: orange;"
      >Расхождений МОЛ: {{ discrepancyMolItems.length }}</span>
    </div>
...
export default {
  ...
  setup() {
    ...
    const newOsItems = computed(() => osItemsBGU.value.filter(item => item.isNew === true));
    ...
    const onSortChanged = () => { /* no-op */ };

    // Кнопка «Перевести» показывается только при расхождении МОЛ
    const actionButtonText = () => 'Перевести';
    const showTransferButton = (row) => row?.hasMolDiscrepancy === true;

    const onActionTriggered = ({ row }) => {
...
.error-message { font-weight: bold; margin-bottom: 5px; }
.error-details { font-size: 0.9em; color: #58151c; word-break: break-all; }

.retry-button, .add-button {
  padding: 8px 15px; background-color: #007bff; color: white; border: none; border-radius: 4px;
  cursor: pointer; transition: background-color 0.2s; display: inline-flex; align-items: center; gap: 5px;
}
.retry-button:hover, .add-button:hover { background-color: #0056b3; }
.add-button:disabled { background-color: #cccccc; cursor: not-allowed; }

.meta-info { background-color: #f0f0f0; border: 1px solid #ddd; font-size: 0.9em; }
.meta-info .item-count { margin-right: 15px; }

.add-new-container { background-color: #e9f5ff; border: 1px solid #b3d7ff; }
.add-error { color: #721c24; margin-top: 10px; word-break: break-word; }
.success-message {
...
.spinner {
  display: inline-block; border: 3px solid rgba(255, 255, 255, 0.3);
  border-radius: 50%; border-top-color: #fff; width: 16px; height: 16px;
  animation: spin 1s ease-in-out infinite; margin-right: 5px;
}
.loading-indicator .spinner { border-top-color: #333; margin-right: 8px; }

@keyframes spin { to { transform: rotate(360deg); } }

:deep(.row-new) { background-color: #fff3cd !important; }
:deep(.row-warning) { background-color: #f8d7da !important; }
:deep(.row-discrepancy-mol) { background-color: #ffe0b2 !important; }
</style>
